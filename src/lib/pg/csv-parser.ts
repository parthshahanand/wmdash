...
export const fetchAllTables = async (year: string = 'FY27'): Promise<TableData[]> => {
    const tableDataPromises = CSV_FILES.map(async (file) => {
        const response = await fetch(`/pg-data/${year}/${file.name}.csv`);
        ...
        return new Promise<TableData>((resolve, reject) => {
            Papa.parse<string[]>(csvString, {
                ...
                complete: (results) => {
                    ...
                    const rows = cleanedData.slice(1).map((row) =>
